...
describe('ColorContext', function() {
  ...
  const itParses = expression => ({
    asUndefined() {
      return it(`parses '${expression}' as undefined`, () => expect(context.getValue(expression)).toBeUndefined());
    },

    asUndefinedColor() {
      return it(`parses '${expression}' as undefined color`, () => expect(context.readColor(expression)).toBeUndefined());
    },

    asInt(expected) {
      return it(`parses '${expression}' as an integer with value of ${expected}`, () => expect(context.readInt(expression)).toEqual(expected));
    },

    asFloat(expected) {
      return it(`parses '${expression}' as a float with value of ${expected}`, () => expect(context.readFloat(expression)).toEqual(expected));
    },

    asIntOrPercent(expected) {
      return it(`parses '${expression}' as an integer or a percentage with value of ${expected}`, () => expect(context.readIntOrPercent(expression)).toEqual(expected));
    },

    asFloatOrPercent(expected) {
      return it(`parses '${expression}' as a float or a percentage with value of ${expected}`, () => expect(context.readFloatOrPercent(expression)).toEqual(expected));
    },

    asColorExpression(expected) {
      return it(`parses '${expression}' as a color expression`, () => expect(context.readColorExpression(expression)).toEqual(expected));
    },

    asColor(...expected) {
      return it(`parses '${expression}' as a color with value of ${jasmine.pp(expected)}`, () => expect(context.readColor(expression)).toBeColor(...Array.from(expected || [])));
    },

    asInvalidColor(...expected) {
      return it(`parses '${expression}' as an invalid color`, () => expect(context.readColor(expression)).not.toBeValid());
    }
  });
  ...
  describe('with variables from a default file', function() {
    ...
    const createVar = function(name, value, path, isDefault=false) {
      if (path == null) { path = `${projectPath}/file.styl`; }
