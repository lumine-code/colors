...
describe('ColorBuffer', function() {
  ...
  const editBuffer = function(text, options={}) {
    ...
    if (!options.noEvent) { return advanceClock(500); }
  };
  ...
  it('creates a color buffer for each editor in the workspace', () => expect(project.colorBuffersByEditorId[editor.id]).toBeDefined());
  ...
  describe('when an editor with a path is not in the project paths is opened', function() {
    ...
    return describe('when the file is not yet saved on disk', function() {
      ...
      it('does not fails when updating the colorBuffer', () => expect(() => colorBuffer.update()).not.toThrow());
    ...
    return describe('when the file is saved and aquires a path', function() {
      describe('that is legible', function() {
        ...
        return it('adds the path to the project paths', function() {
          let needle;
          return expect((needle = 'new-path.styl', project.getPaths().includes(needle))).toBeTruthy();
        });
      });
      ...
      describe('that is not legible', function() {
        ...
        return it('does not add the path to the project paths', function() {
          let needle;
          return expect((needle = 'new-path.styl', project.getPaths().includes(needle))).toBeFalsy();
        });
      });
  ...
  describe('with rapid changes that triggers a rescan', function() {
    ...
    return it('terminates the currently running task', () => expect(colorBuffer.terminateRunningTask).toHaveBeenCalled());
  });
  ...
  describe('when created without a previous state', function() {
    ...
    it('creates the corresponding markers in the text editor', () => expect(colorBuffer.getMarkerLayer().findMarkers().length).toEqual(4));

    it('knows that it is legible as a variables source file', () => expect(colorBuffer.isVariablesSource()).toBeTruthy());

    describe('when the editor is destroyed', () => it('destroys the color buffer at the same time', function() {
      editor.destroy();
    ...
    describe('::getColorMarkerAtBufferPosition', function() {
      describe('when the buffer position is contained in a marker range', () => it('returns the corresponding color marker', function() {
      ...
      return describe('when the buffer position is not contained in a marker range', () => it('returns undefined', () => expect(colorBuffer.getColorMarkerAtBufferPosition([1, 15])).toBeUndefined()));
    });
    ...
    describe('with a buffer with only colors', function() {
      ...
      return describe('when a color marker is edited', function() {
        ...
        it('updates the modified color marker', () => expect(colorBuffer.getColorMarkers().length).toEqual(2));
        ...
        return it('removes the previous editor markers', () => expect(colorBuffer.getMarkerLayer().findMarkers().length).toEqual(2));
      });
    });
    ...
    describe('with a buffer whose scope is not one of source files', function() {
      ...
      return it('does not renders colors from variables', () => expect(colorBuffer.getColorMarkers().length).toEqual(4));
    });
    ...
    return describe('with a buffer in crlf mode', function() {
      ...
      return it('creates a marker for each colors', () => expect(colorBuffer.getValidColorMarkers().length).toEqual(2));
    });
  });
  ...
  describe('with a buffer part of the global ignored files', function() {
    ...
    it('knows that it is part of the ignored files', () => expect(colorBuffer.isIgnored()).toBeTruthy());

    it('knows that it is a variables source file', () => expect(colorBuffer.isVariablesSource()).toBeTruthy());
  ...
  describe('with a buffer not being a variable source', function() {
    ...
    it('knows that it is not part of the source files', () => expect(colorBuffer.isVariablesSource()).toBeFalsy());

    it('knows that it is not part of the ignored files', () => expect(colorBuffer.isIgnored()).toBeFalsy());
  ...
  return describe('when created with a previous state', function() {
    describe('with variables and colors', function() {
      ...
      it('creates markers from the state object', () => expect(colorBuffer.getColorMarkers().length).toEqual(4));
      ...
      it('restores the editor markers', () => expect(colorBuffer.getMarkerLayer().findMarkers().length).toEqual(4));

      return it('restores the ability to fetch markers', function() {
        expect(colorBuffer.findColorMarkers().length).toEqual(4);

        return colorBuffer.findColorMarkers().map((marker) =>
