...
class ColorResultsElement extends HTMLElement {
  ...
  createdCallback() {
    if (CompositeDisposable == null) { ({Range, CompositeDisposable} = require("lumine")); }
    ...
    return this.subscriptions.add(this.subscribeTo(this, '.search-result', {
      click: e => {
        ...
        return lumine.workspace.open(this.pathMapping[pathAttribute]).then(editor => editor.setSelectedBufferRange(range, {autoscroll: true}));
      }
    }
    )
    );
...
document.registerElement('colors-color-results', {
