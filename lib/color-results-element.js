...
const {SpacePenDSL, EventsDelegation, AncestorsMethods} = require('atom-utils');
...
class ColorResultsElement extends HTMLElement {
  ...
  static content() {
    return this.tag('atom-panel', {outlet: 'pane', class: 'preview-pane pane-item'}, () => {
      ...
      return this.ol({outlet: 'resultsList', class: 'search-colors-results results-view list-tree focusable-panel has-collapsable-children native-key-bindings', tabindex: -1});
    });
  }

  createdCallback() {
    if (CompositeDisposable == null) { ({Range, CompositeDisposable} = require('atom')); }
    ...
    return this.subscriptions.add(this.subscribeTo(this, '.search-result', {
      click: e => {
        ...
        return atom.workspace.open(this.pathMapping[pathAttribute]).then(editor => editor.setSelectedBufferRange(range, {autoscroll: true}));
      }
    }
    )
    );
  ...
  setModel(colorSearch) {
    ...
    this.subscriptions.add(this.colorSearch.onDidFindMatches(result => {
      return this.addFileResult(result);
    })
    );

    this.subscriptions.add(this.colorSearch.onDidCompleteSearch(() => {
...
`
  ...
  updateMessage() {
    ...
    return this.previewCount.innerHTML = this.colors > 0 ?
      `\
...
`
  ...
  createFileResult(fileResult) {
    ...
    const pathAttribute = _.escapeAttribute(filePath);
    this.pathMapping[pathAttribute] = filePath;
    const pathName = atom.project.relativize(filePath);
...
<li class="path list-nested-item" data-path="${pathAttribute}">
  ...
  createMatchResult(match) {
    ...
    const textColor = match.color.luma > 0.43 ?
      'black'
    :
      'white';
