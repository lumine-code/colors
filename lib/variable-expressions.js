...
registry.createExpression('colors:scss_params', '^[ \\t]*@(mixin|include|function)\\s+[a-zA-Z0-9\\-_]+\\s*\\([^\\)]+\\)', ['scss', 'sass', 'haml'], function(match, solver) {
...
const sass_handler = function(match, solver) {
  ...
  if (match[1].match(/[-_]/)) {
    ...
    if (match[1] !== all_underscore) {
      solver.appendResult(all_underscore, match[2], 0, match[0].length, {isAlternate: true, isDefault: (match[3] != null)});
    }
    if (match[1] !== all_hyphen) {
      solver.appendResult(all_hyphen, match[2], 0, match[0].length, {isAlternate: true, isDefault: (match[3] != null)});
    }
  }
...
registry.createExpression('colors:scss', '^[ \\t]*(\\$[a-zA-Z0-9\\-_]+)\\s*:\\s*(.*?)(\\s*!default)?\\s*;', ['scss', 'haml'], sass_handler);

registry.createExpression('colors:sass', '^[ \\t]*(\\$[a-zA-Z0-9\\-_]+)\\s*:\\s*([^\\{]*?)(\\s*!default)?\\s*(?:$|\\/)', ['sass', 'haml'], sass_handler);

registry.createExpression('colors:css_vars', '(--[^\\s:]+):\\s*([^\\n;]+);', ['css'], function(match, solver) {
  solver.appendResult(`var(${match[1]})`, match[2], 0, match[0].length);
  return solver.endParsing(match[0].length);
});

registry.createExpression('colors:stylus_hash', '^[ \\t]*([a-zA-Z_$][a-zA-Z0-9\\-_]*)\\s*=\\s*\\{([^=]*)\\}', ['styl', 'stylus'], function(match, solver) {
  let content, name;
  let buffer = '';
  ...
  for (var char of content) {
    if (scopeBegin.test(char)) {
      ...
      scope.pop();
      if (scope.length === 0) { return solver.endParsing(current); }
...
registry.createExpression('colors:stylus', '^[ \\t]*([a-zA-Z_$][a-zA-Z0-9\\-_]*)\\s*=(?!=)\\s*([^\\n\\r;]*);?$', ['styl', 'stylus']);

registry.createExpression('colors:latex', '\\\\definecolor(\\{[^\\}]+\\})\\{([^\\}]+)\\}\\{([^\\}]+)\\}', ['tex'], function(match, solver) {
  let [_, name, mode, value] = Array.from(match);

  value = (() => { switch (mode) {
    case 'RGB': return `rgb(${value})`;
    case 'gray': return `gray(${Math.round(parseFloat(value) * 100)}%)`;
    case 'rgb':
      var values = value.split(',').map(n => Math.floor(n * 255));
      return `rgb(${values.join(',')})`;
    case 'cmyk': return `cmyk(${value})`;
...
registry.createExpression('colors:latex_mix', '\\\\definecolor(\\{[^\\}]+\\})(\\{[^\\}\\n!]+[!][^\\}\\n]+\\})', ['tex'], function(match, solver) {
...
registry.createExpression('colors:javascript', '^[ \\t]*(var|let|const)\\s+([a-zA-Z0-9_]+)\\s*=\\s*[\'\"\`](.*?)[\'\"\`]', ['js'], function(match, solver) {
