...
(Palette = class Palette {
  static deserialize(state) { return new Palette(state.variables); }

  constructor(variables=[]) {
    this.variables = variables;
  }

  getTitle() { return 'Palette'; }

  getURI() { return 'pigments://palette'; }
  ...
  getColorsNames() { return this.variables.map(v => v.name); }

  getColorsCount() { return this.variables.length; }

  eachColor(iterator) { return this.variables.map((v) => iterator(v)); }
