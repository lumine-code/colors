...
class ColorProjectElement extends HTMLElement {
  ...
  static content() {
    const arrayField = (name, label, setting, description) => {
      const settingName = `pigments.${name}`;
      ...
      return this.div({class: 'control-group array'}, () => {
        return this.div({class: 'controls'}, () => {
          ...
          return this.div({class: 'control-wrapper'}, () => {
            this.tag('atom-text-editor', {mini: true, outlet: name, type: 'array', property: name});
            return this.div({class: 'setting-description'}, () => {
              this.div(() => {
                this.raw(`Global config: <code>${atom.config.get(setting != null ? setting : settingName).join(', ')}</code>`);
      ...
      return this.div({class: 'control-group array'}, () => {
        return this.div({class: 'controls'}, () => {
          ...
          return this.div({class: 'control-wrapper'}, () => {
            ...
            return this.div({class: 'setting-description'}, () => {
              this.div(() => {
                this.raw(`Global config: <code>${atom.config.get(setting != null ? setting : settingName)}</code>`);
    ...
    var booleanField = (name, label, description, nested) => {
      return this.div({class: 'control-group boolean'}, () => {
        return this.div({class: 'controls'}, () => {
          this.input({type: 'checkbox', id: `pigments-${name}`, outlet: name});
          this.label({class: 'control-label', for: `pigments-${name}`}, () => {
    ...
    return this.section({class: 'settings-view pane-item'}, () => {
      return this.div({class: 'settings-wrapper'}, () => {
        this.div({class: 'header'}, () => {
          this.div({class: 'logo'}, () => {
            return this.img({src: 'atom://pigments/resources/logo.svg', width: 140, height: 35});
          });
...
`);
  ...
  initializeBindings() {
    const grammar = atom.grammars.grammarForScopeName('source.js.regexp');
  ...
  getURI() { return 'pigments://settings'; }
