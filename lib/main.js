...
let [
  ...
  ColorMarker,
  VariablesCollection, ColorsProvider, ColorsAPI,
  Disposable,
  url, uris
] = Array.from([]);
...
module.exports = {
  ...
  provideAutocomplete() {
    if (ColorsProvider == null) { ColorsProvider = require("./autocomplete-provider"); }
    return new ColorsProvider(this);
  },

  provideAPI() {
    if (ColorsAPI == null) { ColorsAPI = require("./colors-api"); }
  ...
  findColors() {
    ...
    if (!pane) { pane = lumine.workspace.getActivePane(); }
  ...
  showPalette() {
    ...
    return this.project.initialize().then(function() {
      ...
      return lumine.workspace.openURIInPane(uris.PALETTE, pane, {});}).catch(reason => console.error(reason));
  },
  ...
  showSettings() {
    ...
    return this.project.initialize().then(function() {
      ...
      return lumine.workspace.openURIInPane(uris.SETTINGS, pane, {});}).catch(reason => console.error(reason));
  },
  ...
  createReport() {
    ...
    .replace(new RegExp(`${lumine.project.getPaths().join('|')}`, 'g'), '<root>');
  }
};
