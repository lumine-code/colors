 ...
 */
let ColorSearch;
let [Emitter, ColorContext, registry] = Array.from([]);

const { compile } = require("./globs");

module.exports = ColorSearch = class ColorSearch {
  ...
  search() {
    ...
    const promise = lumine.workspace.scan(re, { paths: this.sourceNames }, (m) => {
      const relativePath = lumine.project.relativize(m.filePath);
      ...
      for (var result of m.matches) {
        ...
        if (!(result.color != null ? result.color.isValid() : undefined)) {
          continue;
        }
      ...
      if (m.matches.length > 0) {
        return this.emitter.emit("did-find-matches", m);
      }
    });
