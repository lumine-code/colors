...
module.exports =
(ColorContext = class ColorContext {
  constructor(options={}) {
    let colorVariables, sorted;
    ...
    ({variables, colorVariables, referenceVariable: this.referenceVariable, referencePath: this.referencePath, rootPaths: this.rootPaths, parser: this.parser, colorVars: this.colorVars, vars: this.vars, defaultVars: this.defaultVars, defaultColorVars: this.defaultColorVars, sorted, registry: this.registry, sassScopeSuffix: this.sassScopeSuffix} = options);

    if (variables == null) { variables = []; }
    if (colorVariables == null) { colorVariables = []; }
    if (this.rootPaths == null) { this.rootPaths = []; }
    if (this.referenceVariable != null) { if (this.referencePath == null) { this.referencePath = this.referenceVariable.path; } }
    ...
    if (this.vars == null) {
      ...
      for (v of this.variables) {
        if (!v.default) { this.vars[v.name] = v; }
        if (v.default) { this.defaultVars[v.name] = v; }
      }

      for (v of this.colorVariables) {
        if (!v.default) { this.colorVars[v.name] = v; }
        if (v.default) { this.defaultColorVars[v.name] = v; }
      }
    }
    ...
    if (this.parser == null) {
      if (ColorParser == null) { ColorParser = require('./color-parser'); }
  ...
  sortPaths(a,b) {
    if (this.referencePath != null) {
      ...
      if (a.path === this.referencePath) { return 1; }
      if (b.path === this.referencePath) { return -1; }
      ...
      if (rootA === rootReference) { return 1; }
      if (rootB === rootReference) { return -1; }
  ...
  rootPathForPath(path) {
    for (var root of this.rootPaths) { if (path.indexOf(`${root}/`) === 0) { return root; } }
  }
  ...
  containsVariable(variableName) { let needle;
  return (needle = variableName, this.getVariablesNames().includes(needle)); }

  hasColorVariables() { return this.colorVariables.length > 0; }

  getVariables() { return this.variables; }

  getColorVariables() { return this.colorVariables; }

  getVariablesNames() { return this.varNames != null ? this.varNames : (this.varNames = Object.keys(this.vars)); }

  getVariablesCount() { return this.varCount != null ? this.varCount : (this.varCount = this.getVariablesNames().length); }

  readUsedVariables() {
    const usedVariables = [];
    for (var v of this.usedVariables) { if (!usedVariables.includes(v)) { usedVariables.push(v); } }
    this.usedVariables = [];
    this.resolvedVariables = [];
  ...
  getValue(value) {
    let [realValue, lastRealValue] = Array.from([]);
    const lookedUpValues = [value];

    while ((realValue = this.vars[value] != null ? this.vars[value].value : undefined) && !lookedUpValues.includes(realValue)) {
      this.usedVariables.push(value);
      value = (lastRealValue = realValue);
      lookedUpValues.push(realValue);
    }

    if (lookedUpValues.includes(realValue)) { return undefined; } else { return lastRealValue; }
  }
  ...
  readColor(value, keepAllVariables=false) {
    if (this.usedVariables.includes(value) && !(this.resolvedVariables.includes(value))) { return; }

    const realValue = this.readColorExpression(value);

    if ((realValue == null) || this.usedVariables.includes(realValue)) { return; }

    const scope = (this.colorVars[value] != null) ?
      this.scopeFromFileName(this.colorVars[value].path)
    :
      '*';
    ...
    if (result != null) {
      ...
      if (this.vars[value] != null) { this.usedVariables.push(value); }
    }

    if (result != null) {
      this.resolvedVariables.push(value);
      if (keepAllVariables || !this.usedVariables.includes(value)) {
        result.variables = (result.variables != null ? result.variables : []).concat(this.readUsedVariables());
      }
    }
  ...
  split(value) {
    if (split == null) { ({split, clamp, clampInt} = require('./utils')); }
    return split(value);
  }

  clamp(value) {
    if (clamp == null) { ({split, clamp, clampInt} = require('./utils')); }
    return clamp(value);
  }

  clampInt(value) {
    if (clampInt == null) { ({split, clamp, clampInt} = require('./utils')); }
    return clampInt(value);
  }

  isInvalid(color) { return !Color.isValid(color); }
  ...
  contrast(base, dark=new Color('black'), light=new Color('white'), threshold=0.43) {
    if (dark.luma > light.luma) { [light, dark] = Array.from([dark, light]); }
  ...
  mixColors(color1, color2, amount=0.5, round=Math.floor) {
    if ((color1 == null) || (color2 == null) || !!isNaN(amount)) { return new Color(NaN, NaN, NaN, NaN); }
