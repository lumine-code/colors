...
(VariableExpression = class VariableExpression {
  ...
  constructor({name, regexpString, scopes, priority, handle}) {
    ...
    if (this.handle == null) { this.handle = this.constructor.DEFAULT_HANDLE; }
  }

  match(expression) { return this.regexp.test(expression); }
  ...
  parse(expression) {
    ...
    if (match != null) {
      ...
      const startIndex = lastIndex - matchText.length;
    ...
    if (parsingAborted) { return undefined; } else { return results; }
  }
});
