...
var utils = {
  ...
  clamp(n) { return Math.min(1, Math.max(0, n)); },

  clampInt(n, max=100) { return Math.min(max, Math.max(0, n)); },

  insensitive(s) {
    return s.split(/(?:)/).map(c => `(?:${c}|${c.toUpperCase()})`).join('');
  },
  ...
  countLines(string) { return string.split(/\r\n|\r|\n/g).length; },
  ...
  findClosingIndex(s, startIndex=0, openingChar="[", closingChar="]") {
    ...
    if (nests === 0) { return index - 1; } else { return -1; }
  },
    ...
    while (i < l) {
      ...
      switch(c) {
        ...
        case sep:
          ...
          start = i + 1;
          if (previousStart === start) { break whileLoop; }
          previousStart = start;
          break;
      }
