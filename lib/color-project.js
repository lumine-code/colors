...
const compareArray = function(a,b) {
  ...
  for (let i = 0; i < a.length; i++) { var v = a[i]; if (v !== b[i]) { return false; } }
  return true;
};

module.exports =
(ColorProject = class ColorProject {
  static deserialize(state) {
    ...
    if (atom.inDevMode() && atom.project.getPaths().some(p => p.match(/\/pigments$/))) { markersVersion += '-dev'; }
  ...
  constructor(state={}) {
    ...
    if (Emitter == null) { ({Emitter, CompositeDisposable, Range} = require('atom')); }
    if (VariablesCollection == null) { VariablesCollection = require('./variables-collection'); }

    ({
      includeThemes: this.includeThemes, ignoredNames: this.ignoredNames, sourceNames: this.sourceNames, ignoredScopes: this.ignoredScopes, paths: this.paths, searchNames: this.searchNames, ignoreGlobalSourceNames: this.ignoreGlobalSourceNames, ignoreGlobalIgnoredNames: this.ignoreGlobalIgnoredNames, ignoreGlobalIgnoredScopes: this.ignoreGlobalIgnoredScopes, ignoreGlobalSearchNames: this.ignoreGlobalSearchNames, ignoreGlobalSupportedFiletypes: this.ignoreGlobalSupportedFiletypes, supportedFiletypes: this.supportedFiletypes, variables, timestamp, buffers
    } = state);
    ...
    this.subscriptions.add(this.variables.onDidChange(results => {
      return this.emitVariablesChangeEvent(results);
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.sourceNames', () => {
      return this.updatePaths();
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.ignoredNames', () => {
      return this.updatePaths();
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.ignoredBufferNames', ignoredBufferNames => {
      this.ignoredBufferNames = ignoredBufferNames;
      return this.updateColorBuffers();
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.ignoredScopes', () => {
      return this.emitter.emit('did-change-ignored-scopes', this.getIgnoredScopes());
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.supportedFiletypes', () => {
      this.updateIgnoredFiletypes();
      return this.emitter.emit('did-change-ignored-scopes', this.getIgnoredScopes());
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.ignoreVcsIgnoredPaths', () => {
      return this.loadPathsAndVariables();
    })
    );

    this.subscriptions.add(atom.config.observe('pigments.sassShadeAndTintImplementation', () => {
      return this.colorExpressionsRegistry.emitter.emit('did-update-expressions', {
        registry: this.colorExpressionsRegistry
      });
  }));

    const svgColorExpression = this.colorExpressionsRegistry.getExpression('pigments:named_colors');
    this.subscriptions.add(atom.config.observe('pigments.filetypesForColorWords', scopes => {
    ...
    this.subscriptions.add(this.colorExpressionsRegistry.onDidUpdateExpressions(({name}) => {
      if ((this.paths == null) || (name === 'pigments:variables')) { return; }
    ...
    this.subscriptions.add(this.variableExpressionsRegistry.onDidUpdateExpressions(() => {
      if (this.paths == null) { return; }
      return this.reloadVariablesForPaths(this.getPaths());
    })
    );

    if (timestamp != null) { this.timestamp = new Date(Date.parse(timestamp)); }

    this.updateIgnoredFiletypes();

    if (this.paths != null) { this.initialize(); }
  ...
  observeColorBuffers(callback) {
    for (var id in this.colorBuffersByEditorId) { var colorBuffer = this.colorBuffersByEditorId[id]; callback(colorBuffer); }
  ...
  initialize() {
    if (this.isInitialized()) { return Promise.resolve(this.variables.getVariables()); }
    ...
    .then(() => {
      ...
      if (this.includeThemes) { return this.includeThemesVariables(); }
    }).then(() => {
  ...
  destroy() {
    ...
    if (PathsScanner == null) { PathsScanner = require('./paths-scanner'); }

    this.destroyed = true;

    PathsScanner.terminateRunningTask();

    for (var id in this.colorBuffersByEditorId) { var buffer = this.colorBuffersByEditorId[id]; buffer.destroy(); }
  ...
  reload() {
    return this.initialize().then(() => {
      ...
      if (atom.config.get('pigments.notifyReloads')) {
        return atom.notifications.addSuccess("Pigments successfully reloaded", { dismissable: atom.config.get('pigments.dismissableReloadNotifications'), description: `Found:
...
`);
  ...
  loadPathsAndVariables() {
    ...
    return this.loadPaths().then(({dirtied, removed}) => {
      ...
      if ((this.paths != null) && (dirtied.length > 0)) {
        for (var path of dirtied) { if (!this.paths.includes(path)) { this.paths.push(path); } }
    ...
    .then(paths => {
      ...
      if (results != null) { return this.variables.updateCollection(results); }
    });
  }
  ...
  initializeBuffers() {
    return this.subscriptions.add(atom.workspace.observeTextEditors(editor => {
      ...
      if ((editorPath == null) || this.isBufferIgnored(editorPath)) { return; }

      const buffer = this.colorBufferForEditor(editor);
      if (buffer != null) {
        const bufferElement = atom.views.getView(buffer);
  ...
  hasColorBufferForEditor(editor) {
    if (this.destroyed || (editor == null)) { return false; }
  ...
  colorBufferForEditor(editor) {
    ...
    if (ColorBuffer == null) { ColorBuffer = require('./color-buffer'); }
    ...
    this.subscriptions.add(subscription = buffer.onDidDestroy(() => {
  ...
  colorBufferForPath(path) {
    for (var id in this.colorBuffersByEditorId) {
      var colorBuffer = this.colorBuffersByEditorId[id];
      if (colorBuffer.editor.getPath() === path) { return colorBuffer; }
    }
  }
      ...
      if (this.colorBuffersByEditorId != null) {
        return (() => {
          ...
          for (var editor of atom.workspace.getTextEditors()) {
            if (this.hasColorBufferForEditor(editor) || this.isBufferIgnored(editor.getPath())) { continue; }

            buffer = this.colorBufferForEditor(editor);
            if (buffer != null) {
              var bufferElement = atom.views.getView(buffer);
  ...
  isBufferIgnored(path) {
    ...
    const sources = this.ignoredBufferNames != null ? this.ignoredBufferNames : [];
    for (var source of sources) { if (minimatch(path, source, {matchBase: true, dot: true})) { return true; } }
    return false;
  }
  ...
  getPaths() { return (this.paths != null ? this.paths.slice() : undefined); }

  appendPath(path) { if (path != null) { return this.paths.push(path); } }

  hasPath(path) { return (this.paths != null ? this.paths : []).includes(path); }

  loadPaths(noKnownPaths=false) {
    if (PathsLoader == null) { PathsLoader = require('./paths-loader'); }
  ...
  updatePaths() {
    ...
    return this.loadPaths().then(({dirtied, removed}) => {
      ...
      this.paths = this.paths.filter(p => !removed.includes(p));
      for (var p of dirtied) { if (!this.paths.includes(p)) { this.paths.push(p); } }
  ...
  isVariablesSourcePath(path) {
    ...
    if (minimatch == null) { minimatch = require('minimatch'); }
    path = atom.project.relativize(path);
    const sources = this.getSourceNames();
  ...
  isIgnoredPath(path) {
    ...
    if (minimatch == null) { minimatch = require('minimatch'); }
    path = atom.project.relativize(path);
    const ignoredNames = this.getIgnoredNames();

    for (var ignore of ignoredNames) { if (minimatch(path, ignore, {matchBase: true, dot: true})) { return true; } }
  }

  scopeFromFileName(path) {
    if (scopeFromFileName == null) { scopeFromFileName = require('./scope-from-file-name'); }
  ...
  getPalette() {
    ...
    if (!this.isInitialized()) { return new Palette; }
    return new Palette(this.getColorVariables());
  }

  getContext() { return this.variables.getContext(); }

  getVariables() { return this.variables.getVariables(); }

  getVariableExpressionsRegistry() { return this.variableExpressionsRegistry; }

  getVariableById(id) { return this.variables.getVariableById(id); }

  getVariableByName(name) { return this.variables.getVariableByName(name); }

  getColorVariables() { return this.variables.getColorVariables(); }

  getColorExpressionsRegistry() { return this.colorExpressionsRegistry; }

  showVariableInFile(variable) {
    return atom.workspace.open(variable.path).then(function(editor) {
      if (Range == null) { ({Emitter, CompositeDisposable, Range} = require('atom')); }
  ...
  loadVariablesForPath(path) { return this.loadVariablesForPaths([path]); }
  ...
  getVariablesForPath(path) { return this.variables.getVariablesForPath(path); }

  getVariablesForPaths(paths) { return this.variables.getVariablesForPaths(paths); }

  deleteVariablesForPath(path) { return this.deleteVariablesForPaths([path]); }

  deleteVariablesForPaths(paths) {
    return this.variables.deleteVariablesForPaths(paths);
  }

  reloadVariablesForPath(path) { return this.reloadVariablesForPaths([path]); }

  reloadVariablesForPaths(paths) {
    let promise = Promise.resolve();
    if (!this.isInitialized()) { promise = this.initialize(); }
  ...
  scanPathsForVariables(paths, callback) {
    ...
    if ((paths.length === 1) && (colorBuffer = this.colorBufferForPath(paths[0]))) {
      ...
      return PathsScanner.startTask(paths.map(p => [p, this.scopeFromFileName(p)]), this.variableExpressionsRegistry, results => callback(results));
    }
  }

  loadThemesVariables() {
    if (THEME_VARIABLES == null) { ({THEME_VARIABLES} = require('./uris')); }
    if (ATOM_VARIABLES == null) { ATOM_VARIABLES = require('./atom-variables'); }
  ...
  getRootPaths() { return atom.project.getPaths(); }

  getSassScopeSuffix() {
    let left;
    return (left = this.sassShadeAndTintImplementation != null ? this.sassShadeAndTintImplementation : atom.config.get('pigments.sassShadeAndTintImplementation')) != null ? left : 'compass';
  }
  ...
  getSourceNames() {
    ...
    if (!this.ignoreGlobalSourceNames) {
      let left;
      names = names.concat((left = atom.config.get('pigments.sourceNames')) != null ? left : []);
    }
    return names;
  }
  ...
  setSourceNames(sourceNames=[]) {
    ...
    if ((this.initialized == null) && (this.initializePromise == null)) { return; }
  ...
  getSearchNames() {
    ...
    if (!this.ignoreGlobalSearchNames) {
      let left, left1;
      names = names.concat((left = atom.config.get('pigments.sourceNames')) != null ? left : []);
      names = names.concat((left1 = atom.config.get('pigments.extendedSearchNames')) != null ? left1 : []);
    }
    return names;
  }
  ...
  getIgnoredNames() {
    ...
    if (!this.ignoreGlobalIgnoredNames) {
      ...
      names = names.concat((left1 = atom.config.get('core.ignoredNames')) != null ? left1 : []);
    }
    return names;
  }

  getGlobalIgnoredNames() {
    return __guard__(atom.config.get('pigments.ignoredNames'), x => x.map(function(p) {
      if (/\/\*$/.test(p)) { return p + '*'; } else { return p; }
    }));
  }
  ...
  getIgnoredScopes() {
    ...
    if (!this.ignoreGlobalIgnoredScopes) {
      let left;
      scopes = scopes.concat((left = atom.config.get('pigments.ignoredScopes')) != null ? left : []);
    }
  ...
  getIgnoredFiletypes() {
    ...
    if (!this.ignoreGlobalSupportedFiletypes) {
      let left;
      filetypes = filetypes.concat((left = atom.config.get('pigments.supportedFiletypes')) != null ? left : []);
    }

    if (filetypes.length === 0) { filetypes = ['*']; }

    if (filetypes.some(type => type === '*')) { return []; }

    const scopes = filetypes.map(ext => __guard__(atom.grammars.selectGrammar(`file.${ext}`), x => x.scopeName.replace(/\./g, '\\.'))).filter(scope => scope != null);
  ...
  themesIncluded() { return this.includeThemes; }

  setIncludeThemes(includeThemes) {
    if (includeThemes === this.includeThemes) { return Promise.resolve(); }
  ...
  includeThemesVariables() {
    this.themesSubscription = atom.themes.onDidChangeActiveThemes(() => {
      if (!this.includeThemes) { return; }
