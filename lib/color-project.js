...
const { matchesAny } = require("./globs");
...
(ColorProject = class ColorProject {
  static deserialize(state) {
    ...
    if (lumine.inDevMode() && lumine.project.getPaths().some(p => p.match(/\/colors$/))) { markersVersion += '-dev'; }
    ...
    if (!compareArray(state.globalSourceNames, lumine.config.get('colors.sourceNames')) || !compareArray(state.globalIgnoredNames, lumine.config.get('colors.ignoredNames'))) {
      delete state.variables;
      delete state.buffers;
      delete state.paths;
    }
  ...
  constructor(state={}) {
    ...
    if (Emitter == null) { ({Emitter, CompositeDisposable, Range} = require("lumine")); }
    ...
    this.subscriptions.add(lumine.config.observe('colors.sourceNames', () => {
      return this.updatePaths();
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.ignoredNames', () => {
      return this.updatePaths();
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.ignoredBufferNames', ignoredBufferNames => {
      this.ignoredBufferNames = ignoredBufferNames;
      return this.updateColorBuffers();
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.ignoredScopes', () => {
      return this.emitter.emit('did-change-ignored-scopes', this.getIgnoredScopes());
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.supportedFiletypes', () => {
      this.updateIgnoredFiletypes();
      return this.emitter.emit('did-change-ignored-scopes', this.getIgnoredScopes());
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.ignoreVcsIgnoredPaths', () => {
      return this.loadPathsAndVariables();
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.sassShadeAndTintImplementation', () => {
    ...
    this.subscriptions.add(lumine.config.observe('colors.filetypesForColorWords', scopes => {
    ...
    this.subscriptions.add(this.colorExpressionsRegistry.onDidUpdateExpressions(({name}) => {
      if ((this.paths == null) || (name === 'colors:variables')) { return; }
  ...
  reload() {
    return this.initialize().then(() => {
      ...
      if (lumine.config.get('colors.notifyReloads')) {
        return lumine.notifications.addSuccess("Colors successfully reloaded", { dismissable: lumine.config.get('colors.dismissableReloadNotifications'), description: `Found:
...
`);
      ...
      lumine.notifications.addError("Colors couldn't be reloaded", {detail, stack, dismissable: true});
  ...
  initializeBuffers() {
    return this.subscriptions.add(lumine.workspace.observeTextEditors(editor => {
  ...
  isBufferIgnored(path) {
    ...
    return matchesAny(path, this.ignoredBufferNames != null ? this.ignoredBufferNames : []);
  }
  ...
  isIgnoredPath(path) {
    ...
    path = lumine.project.relativize(path);
    return matchesAny(path, this.getIgnoredNames());
  }
  ...
  showVariableInFile(variable) {
    return lumine.workspace.open(variable.path).then(function(editor) {
      if (Range == null) { ({Emitter, CompositeDisposable, Range} = require("lumine")); }
  ...
  loadThemesVariables() {
    ...
    if (ATOM_VARIABLES == null) { ATOM_VARIABLES = require("./theme-variables"); }
  ...
  getRootPaths() { return lumine.project.getPaths(); }

  getSassScopeSuffix() {
    let left;
    return (left = this.sassShadeAndTintImplementation != null ? this.sassShadeAndTintImplementation : lumine.config.get('colors.sassShadeAndTintImplementation')) != null ? left : 'compass';
  }
  ...
  getSearchNames() {
    ...
    if (!this.ignoreGlobalSearchNames) {
      let left, left1;
      names = names.concat((left = lumine.config.get('colors.sourceNames')) != null ? left : []);
      names = names.concat((left1 = lumine.config.get('colors.extendedSearchNames')) != null ? left1 : []);
    }
    return names;
  }
  ...
  getGlobalIgnoredNames() {
    return __guard__(lumine.config.get('colors.ignoredNames'), x => x.map(function(p) {
  ...
  getIgnoredScopes() {
    ...
    if (!this.ignoreGlobalIgnoredScopes) {
      let left;
      scopes = scopes.concat((left = lumine.config.get('colors.ignoredScopes')) != null ? left : []);
    }
  ...
  getIgnoredFiletypes() {
    ...
    if (!this.ignoreGlobalSupportedFiletypes) {
      let left;
      filetypes = filetypes.concat((left = lumine.config.get('colors.supportedFiletypes')) != null ? left : []);
    }
    ...
    const scopes = filetypes.map(ext => __guard__(lumine.grammars.selectGrammar(`file.${ext}`), x => x.scopeName.replace(/\./g, '\\.'))).filter(scope => scope != null);
