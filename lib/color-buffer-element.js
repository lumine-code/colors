...
class ColorBufferElement extends ColorsElement {
  ...
  getHighlighDecorationCSS(marker, type, backdrop = this.getEditorBackgroundColor()) {
    ...
    // The color is painted once, over the checkerboard that says it is
    // transparent. Painting it as the background color as well — under the
    // checkerboard, as this used to — composites it twice, so a color at half
    // alpha covered three quarters of whatever was behind it.
    if (type === "background") {
      ...
      // checkerboard is faint here for the same reason — it has text over it.
...
`;
  ...
  getGutterDecorationItem(marker) {
    ...
    div.innerHTML = `\
<span style='background-image: linear-gradient(to bottom, ${marker.color.toCSS()} 0%, ${marker.color.toCSS()} 100%), repeating-conic-gradient(rgba(128, 128, 128, 0.45) 0% 25%, transparent 0% 50%); background-size: auto, 10px 10px;' data-marker-id='${marker.id}'></span>\
`;
    return div;
  }
