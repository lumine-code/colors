...
class ColorBufferElement extends ColorsElement {
  ...
  setModel(colorBuffer) {
    ...
    this.subscriptions.add(lumine.config.observe('colors.maxDecorationsInGutter', () => {
      return this.update();
    })
    );

    this.subscriptions.add(lumine.config.observe('colors.markerType', type => {
  ...
  updateHighlightDecorations(type) {
    ...
    if (this.colorBuffer == null) { return; }
...
module.exports =
(ColorBufferElement =
defineElement('colors-markers', ColorBufferElement));
